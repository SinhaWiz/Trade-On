...
async function GET(request) {
    try {
        ...
        // Get top scores
        const topRecords = await recordsCollection.find({}).sort({
            finalBalance: -1
        }).limit(limit).toArray();
        // Build leaderboard with player names
        const leaderboard = await Promise.all(topRecords.map(async (record, index)=>{
            const player = await playersCollection.findOne({
                email: record.playerEmail
            });
            return {
                rank: index + 1,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__d0b2eddd._.js.map